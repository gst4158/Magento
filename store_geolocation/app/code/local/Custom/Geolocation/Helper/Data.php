...
* Mage::helper('custom_geolocation')->customGeolocationInit($_SERVER['REMOTE_ADDR'])
*      access this global: $customGeolocation = Mage::registry('custom_geolocation_user');
*
* Available functions:
* allen TX IP : 206.193.215.123
* /-------------------------------------/
* Mage::helper('custom_geolocation')->getUserContinent( $reader->get($ipAddress) );
* Mage::helper('custom_geolocation')->getUserCountry( $reader->get($ipAddress) );
* Mage::helper('custom_geolocation')->getUserState( $reader->get($ipAddress) );
* Mage::helper('custom_geolocation')->getUserPostal( $reader->get($ipAddress) );
* Mage::helper('custom_geolocation')->getUserCity( $reader->get($ipAddress) );
* Mage::helper('custom_geolocation')->getUserLocation( $reader->get($ipAddress) );
* Mage::helper('custom_geolocation')->getUserGeolocation( $reader->get($ipAddress) );
*/
...
class Custom_Geolocation_Helper_Data extends Mage_Core_Helper_Abstract {

    /**
    * Create global variable from geolocation
    */
    public function customGeolocationInit($ipAddress = '206.193.215.123', $magentoStoresJson){
        // create empty array
        $userGeolocation = array();

        // get geolocation DB
        $databaseFile = Mage::getBaseDir('base').DS.'lib'.DS.'MaxMind'.DS.'GeoIP2'.DS.'vendor'.DS.'geoip2'.DS.'geoip2'.DS.'maxmind-db'.DS.'GeoLite2-City.mmdb';
        ...
        $userDistance = Mage::helper('custom_geolocation/distance')->custom_getStoreUserDist($userGeolocation, $magentoStoresJson);
    ...
    */
    public function custom_getUserDetails($user) {
        $user = array(
